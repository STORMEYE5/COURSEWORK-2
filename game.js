        ...
        const LASER_SPD = 650; // speed of lasers in pixels per second
        const ROID_JAG = 0.18; // jaggedness of the asteroids (0 = none, 1 = lots)
        const ROID_NUM = 5; // starting number of asteroids
        const ROID_SIZE = 120; // starting size of asteroids in pixels
        const ROID_SPD = 30; // max starting speed of asteroids in pixels per second
        const ROID_VERT = 18; // average number of vertices on each asteroid
        const ROIDS_POINTS_LARGE = 20;
        const ROIDS_POINTS_MED = 10;
        const ROIDS_POINTS_SMALL = 5;
        ...
        function gameOver() {
            ...
            currentU.score = currentscore;

            localStorage[currentuser] = JSON.stringify(currentU);
            console.log(currentuser);
        ...
        function keyUp(/** @type {KeyboardEvent} */ ev) {
            ...
            switch(ev.keyCode) {
                case 75: // space bar (allow shooting again)
                    ship.canShoot = true;
                    break;
        ...
        function newAsteroid(x, y, r) {
            ...
            var roid = {
                ...
                r: r,
                offset: [],
                vertex: Math.floor(Math.random() * (ROID_VERT + 3) + ROID_VERT / 4)
            };

            // populate the offsets array
            for (var i = 0; i < roid.vertex; i++) {
        ...
        function newGame() {
            ...
            var score_str = localStorage.getItem(SAVE_SCORE);
            if (score_str == null){
                highscore = 0;
            }else{
                highscore = parseInt(score_str);
            };
            lives = GAME_LIVES;
            ship = newShip();
            newLevel();
        ...
        function update() {
            ...
            ctx.textBaseline = "middle";
            ctx.fillStyle = "RED";
            ctx.font = TEXT_SIZE + "times new roman";
            ctx.fillText(score, canv.width -SHIP_SIZE / 2, SHIP_SIZE);

            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillStyle = "white";
            ctx.font = TEXT_SIZE * 0.1 + "times new roman";
